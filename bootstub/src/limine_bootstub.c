...
static memmap_t get_memmap(void)
{
    ...
    memmap.entries = heap_alloc(memmap.entry_count * sizeof(memmap_entry_t));
...
{
    framebuffer_t *framebuffers = heap_alloc(framebuffer_request.response->framebuffer_count * sizeof(framebuffer_t));
